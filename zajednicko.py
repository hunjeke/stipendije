...
.kol{position:fixed;left:0;right:0;bottom:0;z-index:80;background:var(--tinta);
  color:#F2F4F8;padding:1rem 1.15rem;display:none}
...
.kol a{color:#F2F4F8;text-decoration:underline}
.kol-gumbi{display:flex;gap:.6rem;flex-shrink:0}
.kol button{font-family:"Plex",sans-serif;font-size:.87rem;
  padding:.55rem 1rem;border:1.5px solid #F2F4F8;background:none;
  color:#F2F4F8;cursor:pointer}
...
CSS = KOLACICI_CSS + """
:root{
  --papir:#F2F4F8;      /* papir s plavim tonom */
  --karta:#FFFFFF;
  --tinta:#141B2D;      /* mornarska tinta */
  --tinta-2:#565F76;    /* prigusena */
  --linija:#DBE0EA;
  --plava:#1D4ED8;      /* glavna boja: veze, naglasci, znak */
  --plava-t:#1739A0;    /* tamnija, za hover */
  --otvoreno:#15803D;   /* zelena znaci samo: natjecaj je otvoren */
  --hitno:#B91C1C;      /* rok istjece */
}
...
.nav a{font-size:.86rem;color:var(--tinta-2);text-decoration:none;
  ...
  line-height:1.2}
.nav a:hover{color:var(--plava)}
.nav a.tu{color:var(--plava);border-bottom-color:var(--plava)}
